...
function Square({value, onSquareClick, highlight, avatars, theme}){
  return(
   <button
      ...
      aria-label={`Square ${value ? `occupied by ${value}` : 'empty'}`}
    >
      {value && (
        <img 
          src={avatars[value]} 
...
export default function Game(){
  ...
  // SIDEBAR PANELS STATE - Collapse all panels on mobile by default
  const [openPanels, setOpenPanels] = useState({
    gameMode: !isMobile, // Open on desktop, closed on mobile
    theme: false,
    avatars: false,
    history: false,
    scoreboard: !isMobile, // Open on desktop, closed on mobile
    stats: false
  });
  ...
  // Auto-close all panels on mobile when game starts
  useEffect(() => {
    if (isMobile) {
      setOpenPanels({
        gameMode: false,
    ...
    <div className={`game-container ${theme}`}>
        ...
        <div className="main-game-area">
          <div className="game-board-wrapper">
            ...
            <div className={`game-board ${CalculateWinner(currentSquares) ? "winner" : ""} ${theme}`}>
              ...
              {gameMode === 'ai' && isMobile && (
                <div className="mobile-ai-indicator">
                  <span className="ai-turn">
                    {isAiThinking ? '🤖 AI Thinking...' : xIsNext === (aiPlayer === 'X') ? '🤖 AI Turn' : '👤 Your Turn'}
                  </span>
                </div>
              )}
      ...
      {isMobile && (
        <div className="mobile-quick-actions">
          <button 
            className={`mobile-quick-btn ${theme}`}
            onClick={() => togglePanel('gameMode')}
          >
            🎮 Mode
          </button>
          <button 
